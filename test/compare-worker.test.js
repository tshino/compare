...
describe('compareWorker', () => {
    ...
    describe('calcVectorscope', () => {
        ...
        const nonzeroElems = function(array, offx, offy) {
            const result = [];
            for (let j = 0; j < array.length; j += 320*320) {
                const elems = [];
                for (let i = 0; i < 320*320; i++) {
                    if (array[j+i] !== 0) {
                        const x = i % 320, y = (i - x) / 320;
                        elems.push([x-offx,y-offy,array[j+i]]);
                    }
                }
                result.push(elems);
            }
        ...
        it('should create vectorscope image: G-B with colormap', () => {
            ...
            assert.strictEqual(responseData.result.colorMap.length, 320*320*3);
            assert.deepStrictEqual(nonzeroElems(responseData.result.colorMap, 32, 287), [
                [[0,0,85*4]], // R
                [[32,0,32*1], [64,0,64*1], [96,0,96*1], [128,0,128*1]], // G
                [[0,-192,192*2], [0,-128,128*2], [0,-64,64*2]] // B
            ]);
        });
        it('should create vectorscope image: G-R with colormap', () => {
            runTask({
                cmd: 'calcVectorscope',
                type: 3, // G-R
                color: true, // colorMode
            ...
            assert.strictEqual(responseData.result.dist.length, 320*320);
            assert.deepStrictEqual(nonzeroElems(responseData.result.dist, 32, 287), [[
                [0,-85,4], [0,0,8], [32,0,1], [64,0,1], [96,0,1], [128,0,1]
            ]]);
            assert.strictEqual(responseData.result.colorMap.length, 320*320*3);
            assert.deepStrictEqual(nonzeroElems(responseData.result.colorMap, 32, 287), [
                [[0,-85,85*4]],
                [[32,0,32*1], [64,0,64*1], [96,0,96*1], [128,0,128*1]],
                [[0,0,192*2+128*2+64*2]] // B
            ]);
        });
        it('should create vectorscope image: G-R in linear RGB', () => {
            runTask({
                cmd: 'calcVectorscope',
                type: 3, // G-R
                color: false, // colorMode
            ...
            assert.strictEqual(responseData.result.dist.length, 320*320);
            assert.deepStrictEqual(nonzeroElems(responseData.result.dist, 32, 287), [[
                [0,-23,4], [0,0,8], [4,0,1], [13,0,1], [30,0,1], [55,0,1]
            ]]);
        });
        it('should create vectorscope image: B-R with colormap', () => {
            runTask({
                cmd: 'calcVectorscope',
                type: 4, // B-R
                color: true, // colorMode
            ...
            assert.strictEqual(responseData.result.dist.length, 320*320);
            assert.deepStrictEqual(nonzeroElems(responseData.result.dist, 32, 287), [[
                [0,-85,4], [0,0,6], [64,0,2], [128,0,2], [192,0,2]
            ]]);
            assert.strictEqual(responseData.result.colorMap.length, 320*320*3);
            assert.deepStrictEqual(nonzeroElems(responseData.result.colorMap, 32, 287), [
                [[0,-85,85*4]],
                [[0,0,32+64+96+128]],
                [[64,0,64*2], [128,0,128*2], [192,0,192*2]]
            ]);
        });
        it('should create vectorscope image: B-R in linear RGB', () => {
            runTask({
                cmd: 'calcVectorscope',
                type: 4, // B-R
                color: false, // colorMode
            ...
            assert.strictEqual(responseData.result.dist.length, 320*320);
            assert.deepStrictEqual(nonzeroElems(responseData.result.dist, 32, 287), [[
                [0,-23,4], [0,0,6], [13,0,2], [55,0,2], [134,0,2]
            ]]);
        });
    });
    ...
    describe('calcToneCurve', () => {
        ...
        it('should calculate tone curve data: with orientation', () => {
        ...
        // image4 is identical to image2 but magnified by 2
        const image4 = { width: 6, height: 4, data: [
            10,10,10,10, 10,10,10,10, 20,20,20,20, 20,20,20,20, 30,30,30,30, 30,30,30,30,
            10,10,10,10, 10,10,10,10, 20,20,20,20, 20,20,20,20, 30,30,30,30, 30,30,30,30,
            30,30,30,30, 30,30,30,30, 30,30,30,30, 30,30,30,30, 30,30,30,30, 30,30,30,30,
            30,30,30,30, 30,30,30,30, 30,30,30,30, 30,30,30,30, 30,30,30,30, 30,30,30,30
        ]};
        it('should calculate tone curve data: with resize', () => {
            const task = {
                cmd: 'calcToneCurve',
                type: 1,
                auxTypes: [0],
                imageData: [image1, image4]
            };
            runTask(task);
    ...
    describe('calcOpticalFlow', () => {
        ...
        it('should calculate optical flow between different size and rotated images', () => {
            const makeRegion = compareImageUtil.makeRegion;
            const fill = compareImageUtil.fill;
            const image1 = compareImageUtil.makeImage(100, 80);
            const image2 = compareImageUtil.makeImage(160, 200);
            fill(image1, 255, 255, 255, 255);
            fill(makeRegion(image1, 20, 30, 50, 30), 0, 0, 0, 255);
            fill(image2, 255, 255, 255, 255);
            fill(makeRegion(image2, 40, 40, 60, 100), 0, 0, 0, 255);
            const task = {
                cmd: 'calcOpticalFlow',
                imageData: [image1, image2],
                options: {
